...
task_labels <- c("Material Edit", "Translation Edit", "Light Edit")
method_labels <- c("Incremental", "Indirection\nAware", "Focus\nBased")

color_tasks <- c("#009E73", "#56B4E9", "#E69F00")
#color_methods <- c("#e41a1c", "#999999", "#377eb8")
color_methods <- c("#ffb000", "#dc267f", "#648fff")
...
data_relevant <- data_all[,20:112]
# colnames(data_relevant)
data_general <- data_relevant[,c(80:93)]

factor_rows <- c("preference", "experience", "status", "frequency", "gender")
...
for (question in question_labels)
{
  for (task in 1:3)
  {
    data_tlx[paste0(question,"_",task,"a")] <- data_relevant[,paste0(question, "_",task,"a")]
...
tlx_favorites$fav_stated <- factor(tlx_favorites$fav_stated, levels=c("C", "E", "F"))
